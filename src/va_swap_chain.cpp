...
VaSwapChain::VaSwapChain(VaDevice &deviceRef, VkExtent2D extent,
                         std::shared_ptr<VaSwapChain> previous)
    : device{deviceRef}, windowExtent{extent},
...
void VaSwapChain::createRenderPass() {
  ...
  colorAttachment.samples = device.msaaSampleCount;
  ...
  depthAttachment.samples = device.msaaSampleCount;
...
VkExtent2D
VaSwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
