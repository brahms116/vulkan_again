 ...
 */
VkDeviceSize VaBuffer::getAlignment(VkDeviceSize instanceSize,
...
VaBuffer::VaBuffer(VaDevice &device, VkDeviceSize instanceSize,
                   uint32_t instanceCount, VkBufferUsageFlags usageFlags,
                   VkMemoryPropertyFlags memoryPropertyFlags,
                   VkDeviceSize minOffsetAlignment)
    : vaDevice{device}, instanceSize{instanceSize},
...
VaBuffer::~VaBuffer() {
  unmap();
  vkDestroyBuffer(vaDevice.device(), buffer, nullptr);
  vkFreeMemory(vaDevice.device(), memory, nullptr);
}
...
VkResult VaBuffer::map(VkDeviceSize size, VkDeviceSize offset) {
  ...
  return vkMapMemory(vaDevice.device(), memory, offset, size, 0, &mapped);
}
...
void VaBuffer::unmap() {
  if (mapped) {
    vkUnmapMemory(vaDevice.device(), memory);
    mapped = nullptr;
  }
}
 ...
 *
 */
void VaBuffer::writeToBuffer(void *data, VkDeviceSize size,
                             VkDeviceSize offset) {
...
VkResult VaBuffer::flush(VkDeviceSize size, VkDeviceSize offset) {
  ...
  return vkFlushMappedMemoryRanges(vaDevice.device(), 1, &mappedRange);
}
...
VkResult VaBuffer::invalidate(VkDeviceSize size, VkDeviceSize offset) {
  ...
  mappedRange.offset = offset;
  mappedRange.size = size;
  return vkInvalidateMappedMemoryRanges(vaDevice.device(), 1, &mappedRange);
}
 ...
 */
VkDescriptorBufferInfo VaBuffer::descriptorInfo(VkDeviceSize size,
                                                VkDeviceSize offset) {
 ...
 *
 */
VkResult VaBuffer::flushIndex(int index) {
